...
- **The engine only accepts a field of view from 30 to 110.** Found from the first run of the real controls, which logged `Custom FOV must be within [30.0, 110.0]` for the bolt rifle's 24. The spike accepted 1 to 179 before that, and now says 30 to 110.
- **A held right-click blocks the left-click.** The first controls made aim a hold (an item in use), and the owner could not fire while aiming: the game sends no attack input while an item is in use, as with a drawn bow. Aim is now a tap-to-toggle on plain items (see GUN-CONTROLS.md).
