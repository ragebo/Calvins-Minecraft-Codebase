...
3. **A role tag typed by hand reaches the record on the next poll, not at once.** Game logic reads the record, not the tag, and `core/state` looks for tags that changed without it about once a second (`STATE_SYNC.reconcileIntervalTicks`, 20 ticks). So `/tag B add outlaw` followed at once by `bounty:lockpick` can still answer `Only outlaws can attempt this.`, and a second later it does not. `/scriptevent rae:adopt` makes the check immediately and lists what each record says (see `STATE-ADOPT.md`). (This replaces the first version of this task, where a hand-set tag was only picked up when the world loaded.)
